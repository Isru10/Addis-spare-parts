...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { 
  ShieldCheck, Truck, RotateCcw, Minus, Plus, ShoppingCart, Check
} from "lucide-react";
import ProductImageGallery from "@/components/product/ProductImageGallery";
import { cn } from "@/lib/utils";
...
export default function ProductClientPage({ product }: ProductClientPageProps) {
  ...
  // Grouping Logic
  // We assume the first attribute (e.g. "Condition" or "Size") is the primary differentiator
  const primaryAttributeName = product.variants[0]?.attributes[0]?.name || "Option";
  ...
  const handleAddToCart = () => {
    if (selectedVariant) {
      dispatch(addItem({ 
        product, 
        selectedVariant, // <--- This sends the SPECIFIC choice to the cart
        quantity 
      }));
    }
  };
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-10 xl:gap-16">
      ...
      <div className="flex flex-col gap-6">
        ...
        {/* VARIANT SELECTOR (The important part) */}
        {product.variants.length > 1 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-foreground">
                Select {primaryAttributeName}: <span className="text-primary font-bold">{selectedVariant?.attributes[0]?.value}</span>
              </span>
            </div>
              ...
              {product.variants.map((variant) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "px-4 py-2 rounded-md border text-sm font-medium transition-all relative min-w-[3rem]",
                      isSelected 
                        ? "border-primary bg-primary text-primary-foreground shadow-sm" 
                        : "bg-background hover:bg-muted text-foreground",
                      isVariantOutOfStock && "opacity-50 cursor-not-allowed bg-muted/50 text-muted-foreground line-through decoration-muted-foreground/50"
                    )}
                  >
                    {attrValue}
                  </button>
        ...
        <div className="space-y-4 pt-4">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <span className="text-sm text-muted-foreground">
              {selectedVariant ? (
                 isOutOfStock ? <span className="text-red-500 font-medium">Unavailable</span> : `${selectedVariant.stock} available`
              ) : "Select options"}
            </span>
          </div>

          <div className="flex flex-col sm:flex-row gap-3">
            <Button size="lg" className="flex-1 h-12 text-base font-bold shadow-md" onClick={handleAddToCart} disabled={isOutOfStock || !selectedVariant}>
              <ShoppingCart className="mr-2 h-5 w-5" /> Add to Cart
            </Button>
            {/* Added for full complete look */}
        ...
        {/* ... Trust Badges & Tabs (Same as before) ... */}
        {/* (Rest of the component remains the same, assuming it's already implemented) */}
        
        {/* COPIED TABS FROM PREVIOUS RESPONSE FOR COMPLETENESS IF NEEDED */}
        <Tabs defaultValue="desc" className="w-full mt-4">
          <TabsList className="w-full justify-start border-b rounded-none h-auto p-0 bg-transparent">
            <TabsTrigger value="desc" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-4 py-2">Description</TabsTrigger>
            <TabsTrigger value="specs" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-4 py-2">Compatibility & Specs</TabsTrigger>
          </TabsList>
          <TabsContent value="desc" className="pt-4 animate-in fade-in">
             <div className="prose dark:prose-invert max-w-none text-muted-foreground text-sm leading-relaxed">
               <p className="whitespace-pre-line">{product.description}</p>
             </div>
          </TabsContent>
          <TabsContent value="specs" className="pt-4 animate-in fade-in">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="border rounded-lg p-4 bg-muted/10">
                <h4 className="font-semibold mb-2 text-sm">Vehicle Compatibility</h4>
                {product.modelCompatibility && product.modelCompatibility.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {product.modelCompatibility.map((model, i) => (
                      <Badge key={i} variant="secondary" className="bg-background">{model}</Badge>
                    ))}
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground">Universal fit.</p>
                )}
              </div>
              <div className="border rounded-lg p-4 bg-muted/10">
                 <h4 className="font-semibold mb-2 text-sm">Technical Specifications</h4>
                 <ul className="text-sm space-y-2">
                   <li className="flex justify-between border-b border-muted-foreground/10 pb-1"><span className="text-muted-foreground">Brand</span><span className="font-medium">{product.brand}</span></li>
                   <li className="flex justify-between border-b border-muted-foreground/10 pb-1"><span className="text-muted-foreground">SKU</span><span className="font-medium">{selectedVariant?.sku}</span></li>
                   {product.specs?.map((spec, i) => (
                       <li key={i} className="flex justify-between border-b border-muted-foreground/10 pb-1 last:border-0"><span className="text-muted-foreground">{spec.name}</span><span className="font-medium">{spec.value}</span></li>
                   ))}
                 </ul>
              </div>
            </div>
