...
import InsurerRegistrationForm from "@/components/insurer/InsurerRegistrationForm";

export default async function RegisterPage() {
  const user = await getCurrentUser();

  if (!user) return redirect("/login?callbackUrl=/insurer/register");

  // Check if profile already exists
  await dbConnect();
  const existingProfile = await InsurerProfile.findOne({ userId: user.id });

  if (existingProfile) {
    if (existingProfile.status === 'Approved') {
      redirect("/insurer/dashboard");
    } else if (existingProfile.status === 'Pending') {
      redirect("/insurer/pending");
    }
  }

  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
       <InsurerRegistrationForm /> 
    </div>
  );
}